...
async def extract_keywords_with_prompt(
    ...
    try:
        ...
        try:
            ...
            logger.warning("Failed to parse JSON from response: %s", response.output_text[:100])
            return []
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Output path for evaluation results JSON file (default: evaluation_results/results.json)",
    )
    parser.add_argument(
        "--delay",
        type=float,
